...
class Tambah{
  ...
  async getResponse(input: string) {
    ...
    // check question alr exist
    const existingQuestion = await this.db.question.findFirst({ where: { question: question.trim() } });
    ...
    try {
      await this.db.question.create({ data: { question: question.trim(), answer: answer.trim() } });
